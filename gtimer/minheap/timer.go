...
func (t *Timer) add(td *TimerData) {
	...
	if td.index == 0 {
		...
		t.signal.Reset(d)
		if Debug {
			log.Printf("gtimer: add reset delay %d ms", int64(d)/int64(time.Millisecond))
		}
	}
	if Debug {
		log.Printf("gtimer: push item key: %s, expire: %s, index: %d", td.Key, td.ExpireString(), td.index)
	}
	return
}
...
func (t *Timer) del(td *TimerData) {
	...
	if i < 0 || i > last || t.timers[i] != td {
		...
		if Debug {
			log.Printf("gtimer del i: %d, last: %d, %p", i, last, td)
		}
		return
	}
	if i != last {
	...
	if Debug {
		log.Printf("gtimer: remove item key: %s, expire: %s, index: %d", td.Key, td.ExpireString(), td.index)
	}
	return
}
...
func (t *Timer) expire() {
	...
	for {
		if len(t.timers) == 0 {
			d = infiniteDuration
			if Debug {
				log.Printf("gtimer: no other instance")
			}
			break
		}
		td = t.timers[0]
		...
		t.lock.Unlock()
		if fn == nil {
			log.Printf("[WARN]expire gtimer no fn")
		} else {
			if Debug {
				log.Printf("gtimer key: %s, expire: %s, index: %d expired, call fn", td.Key, td.ExpireString(), td.index)
			}
			fn(arg)
		}
		t.lock.Lock()
	...
	t.signal.Reset(d)
	if Debug {
		log.Printf("gtimer: expier reset delay %d ms", int64(d)/int64(time.Millisecond))
	}
	t.lock.Unlock()
	return
}
