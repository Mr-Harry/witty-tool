...
func (t *Timer) add(td *TimerData) {
	...
	if td.index == 0 {
		...
		t.signal.Reset(d)
		if Debug {
			log.Println("gtimer: add reset delay %d ms", int64(d)/int64(time.Millisecond))
		}
	}
	if Debug {
		log.Println("gtimer: push item key: %s, expire: %s, index: %d", td.Key, td.ExpireString(), td.index)
	}
	return
}
...
func (t *Timer) del(td *TimerData) {
	...
	if i < 0 || i > last || t.timers[i] != td {
		...
		if Debug {
			log.Println("gtimer del i: %d, last: %d, %p", i, last, td)
		}
		return
	}
	if i != last {
	...
	if Debug {
		log.Println("gtimer: remove item key: %s, expire: %s, index: %d", td.Key, td.ExpireString(), td.index)
	}
	return
}

// Set update gtimer data.
...
func (t *Timer) expire() {
	...
	for {
		if len(t.timers) == 0 {
			d = infiniteDuration
			if Debug {
				log.Println("gtimer: no other instance")
			}
			break
		}
		td = t.timers[0]
		...
		t.lock.Unlock()
		if fn == nil {
			log.Println("expire gtimer no fn")
		} else {
			if Debug {
				log.Println("gtimer key: %s, expire: %s, index: %d expired, call fn", td.Key, td.ExpireString(), td.index)
			}
			fn()
		}
		t.lock.Lock()
	...
	t.signal.Reset(d)
	if Debug {
		log.Println("gtimer: expier reset delay %d ms", int64(d)/int64(time.Millisecond))
	}
	t.lock.Unlock()
	return
}
